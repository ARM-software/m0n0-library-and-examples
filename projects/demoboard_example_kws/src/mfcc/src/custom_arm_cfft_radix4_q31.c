...
void custom_arm_radix4_butterfly_inverse_q31(
    q31_t * pSrc,
    uint32_t fftLen,
    q31_t * pCoef,
    uint32_t twidCoefModifier);

void custom_arm_radix4_butterfly_q31(
    q31_t * pSrc,
    uint32_t fftLen,
    q31_t * pCoef,
    uint32_t twidCoefModifier);

void custom_arm_bitreversal_q31(
    q31_t * pSrc,
    uint32_t fftLen,
 ...
 * @deprecated Do not use this function.  It has been superseded by \ref custom_arm_cfft_q31 and will be removed
  ...
  q31_t * pSrc)
{
  if (S->ifftFlag == 1U)
  {
    ...
    custom_arm_radix4_butterfly_inverse_q31(pSrc, S->fftLen, S->pTwiddle, S->twidCoefModifier);
  }
  else
  {
    /* Complex FFT radix-4 */
    custom_arm_radix4_butterfly_q31(pSrc, S->fftLen, S->pTwiddle, S->twidCoefModifier);
  }

  if (S->bitReverseFlag == 1U)
  {
    /*  Bit Reversal */
    custom_arm_bitreversal_q31(pSrc, S->fftLen, S->bitRevFactor, S->pBitRevTable);
  }
